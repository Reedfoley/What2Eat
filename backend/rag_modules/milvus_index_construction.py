...
class MilvusIndexConstructionModule:
    ...
    def __init__(self, 
                ...
                dimension: int = 512,
                model_name: str = "F:/datawhale/ALL-in-RAG/models/bge/bge-small-zh-v1.5"):
        """
        初始化Milvus索引构建模块
        ...
        self._setup_client() # 初始化Milvus客户端
    ...
    def _create_collection_schema(self) -> CollectionSchema:
        ...
        # 定义字段
        fields = [
            FieldSchema(name="id", dtype=DataType.VARCHAR, max_length=150, is_primary=True), # 主键ID
            FieldSchema(name="vector", dtype=DataType.FLOAT_VECTOR, dim=self.dimension), # 向量字段
            FieldSchema(name="text", dtype=DataType.VARCHAR, max_length=15000), # 文本字段
            FieldSchema(name="node_id", dtype=DataType.VARCHAR, max_length=100), # 节点ID 
            FieldSchema(name="recipe_name", dtype=DataType.VARCHAR, max_length=300), # 菜谱名称
            FieldSchema(name="node_type", dtype=DataType.VARCHAR, max_length=100), # 节点类型
            FieldSchema(name="category", dtype=DataType.VARCHAR, max_length=100), # 菜谱分类
            FieldSchema(name="cuisine_type", dtype=DataType.VARCHAR, max_length=200), # 菜系
            FieldSchema(name="difficulty", dtype=DataType.INT64), # 难度等级
            FieldSchema(name="doc_type", dtype=DataType.VARCHAR, max_length=50), # 文档类型
            FieldSchema(name="chunk_id", dtype=DataType.VARCHAR, max_length=150), # 分块ID
            FieldSchema(name="parent_id", dtype=DataType.VARCHAR, max_length=100) # 父节点ID
        ]
